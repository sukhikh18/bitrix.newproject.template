...
<div class="search-page">
	<?/*<section class="item-list item-list__search-form">
...
<?elseif($count>0):?>
	...
	$arParams["DISPLAY_PREVIEW_TEXT"] = 'Y';
	$arParams["MORE_LINK_TEXT"] = ''; // Подробнее
	$arParams['HIDE_GLOBAL_LINK'] = "Y";

	$itemClass = function_exists('get_item_class') ?
		get_item_class( 'item', (!empty($arParams['COLUMNS'])) ? $arParams['COLUMNS'] : 0 ) : 'item';
	?>
	<section class="item-list item-list_type_<?=!empty($arParams["IBLOCK_CODE"]) ? $arParams["IBLOCK_CODE"] : '';?>">
		<div class="<?=$arParams['ROW_CLASS'] ? $arParams['ROW_CLASS'] : 'row';?>">
	<?foreach($arResult["SEARCH"] as $arItem):?>
	<?
	$hasPicture = false;
	// @todo replace to result modifier
	$arItem["NAME"] = str_replace(array('<b>', '</b>'), array('<strong>', '</strong>'), $arItem["TITLE_FORMATED"]);
	$arItem["PREVIEW_TEXT"] = str_replace(array('<b>', '</b>'), array('<strong>', '</strong>'), $arItem["BODY_FORMATED"]);
	$arItem["DETAIL_PAGE_URL"] = $arItem["URL"];

	// $arItem["PREVIEW_PICTURE"]["SRC"]
	if( $arItem['DETAIL_PAGE_URL'] && "Y" == $arParams["DISPLAY_MORE_LINK"] )
		$arItem['DETAIL_PAGE_URL_HTML'] = '<a class="item__more" href="' .$arItem['DETAIL_PAGE_URL']. '">' .$arParams["MORE_LINK_TEXT"]. '</a>';

	if( "Y" == $arParams['HIDE_GLOBAL_LINK'] )
        $arItem['DETAIL_PAGE_URL'] = '';

	unset( $arItem["TITLE_FORMATED"] );
	unset( $arItem["URL"] );
	unset( $arItem["BODY_FORMATED"] );

	$file = new Main\IO\File( $documentRoot . $templateFolder . '/../../news.list/.default/item-content.php' ); // $documentRoot
	...
	<?if($arParams["DISPLAY_BOTTOM_PAGER"] != "N"  && $arParams["PAGE_RESULT_COUNT"] <= count($arResult["SEARCH"]))
		echo $arResult["NAV_STRING"]?>
	<br />
	<p>
	<? // sort by
